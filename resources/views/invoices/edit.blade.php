    ...
    <form method="POST" action="{{ route('invoices.update', $invoice->id) }}" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/80 space-y-5">
            <h3 class="font-bold text-slate-900 text-lg border-b border-slate-100 pb-3 flex items-center justify-between">
                ...
                <span class="text-xs font-mono font-normal text-slate-400">{{ $invoice->invoice_number }}</span>
            </h3>
            ...
            <div>
                ...
                <input type="text" name="customer_name" value="{{ old('customer_name', $invoice->customer_name) }}"
                       placeholder="Contoh: Bengkel Honda Jaya / Budi"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="payment_status" required class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none">
                        <option value="paid"    {{ old('payment_status', $invoice->payment_status) === 'paid'    ? 'selected' : '' }}>Paid (Lunas)</option>
                        <option value="unpaid"  {{ old('payment_status', $invoice->payment_status) === 'unpaid'  ? 'selected' : '' }}>Unpaid (Belum Lunas)</option>
                        <option value="overdue" {{ old('payment_status', $invoice->payment_status) === 'overdue' ? 'selected' : '' }}>Overdue (Jatuh Tempo)</option>
                    </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-200/80 space-y-3">
                ...
                <textarea name="notes" rows="4" placeholder="Catatan tambahan, instruksi pembayaran, dll..."
                          class="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-blue-500 outline-none">{{ old('notes', $invoice->notes) }}</textarea>
            </div>
            ...
            <div class="bg-slate-900 text-white rounded-2xl p-6 shadow-lg flex flex-col justify-between space-y-4">
                ...
                <div>
                    ...
                    <div class="mt-4 flex flex-col space-y-2">
                        <button type="submit"
                                class="w-full py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-extrabold text-sm rounded-xl transition shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                            💾 Simpan Perubahan Invoice
                        </button>
                        <a href="{{ route('invoices.show', $invoice->id) }}"
                           class="w-full py-2.5 bg-slate-700 hover:bg-slate-600 text-slate-300 font-bold text-xs rounded-xl transition text-center block">
...
<script>
    function invoiceEditForm() {
        return {
            ...
            // Pre-fill existing items from the database
            items: @json($existingItems),


            addRow() {
                const defaultDate = document.querySelector('[name="issue_date"]')?.value || '';
                this.items.push({ product_id: '', displayText: '', open: false, order_date: defaultDate, quantity: 1, unit_price: 0, subtotal: 0 });
            },
            ...
            getFilteredProducts(search) {
                if (!search || search.trim() === '') return this.availableProducts;
